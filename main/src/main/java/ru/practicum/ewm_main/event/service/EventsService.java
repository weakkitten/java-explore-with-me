            ...
            if (text == null) {
                ...
                if (categories == null) {
                    if (paid) {
                        if (onlyAvailable) {
                            if (sort.equals("VIEWS")) {
                                eventsList = repository.getEventsTextAndAvailableAndPaid(text, start, end, pageable);
                            } else {
                                eventsList = repository.getEventsTextAndAvailableAndPaidByDate(text, start,
                                        end, pageable);
                            }
                        } else {
                            if (sort.equals("VIEWS")) {
                                eventsList = repository.getEventsTextAndPaid(text, start, end, pageable);
                            } else {
                                eventsList = repository.getEventsTextAndPaidByDate(text, start, end, pageable);
                            }
                        }
                    } else {
                        if (onlyAvailable) {
                            if (sort.equals("VIEWS")) {
                                eventsList = repository.getEventsTextAndAvailable(text, start, end, pageable);
                            } else {
                                eventsList = repository.getEventsTextAndAvailableByDate(text, start, end, pageable);
                            }
                        } else {
                            if (sort.equals("VIEWS")) {
                                eventsList = repository.getEventsText(text, start, end, pageable);
                            } else {
                                eventsList = repository.getEventsTextByDate(text, start, end, pageable);
                            }
                        }
                    }
                } else {//с текстом, категорией, без времени
                    if (paid) {
                        ...
                        if (onlyAvailable) {
                            ...
                            if (sort.equals("VIEWS")) {
                                eventsList = repository.getEventsTextAndCategories(text, categories, start, end, pageable);
                            } else {
                                eventsList = repository.getEventsTextAndCategoriesByDate(text, categories,
                                        start, end, pageable);
                            }
