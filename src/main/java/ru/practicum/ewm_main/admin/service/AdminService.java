...
public class AdminService {
    ...
    public ResponseEntity<Object> getEvents(List<Integer> users,
        ...
        eventsList = eventsRepository.getEventsWithStates(stateList,
                LocalDateTime.parse(rangeStart, formatter),
                LocalDateTime.parse(rangeEnd, formatter),
                PageRequest.of(from / size, size));
      if (users.get(0) == 0 && categories.get(0) == 0) {
          EventFullDto dto = EventFullDto.builder().build();
          return ResponseEntity.ok(List.of(dto));
      }
      /*if (states == null) {
            eventsList = eventsRepository.getEventsWithUsersAndCategoriesWithoutStates(users,
                    categories,
                    LocalDateTime.parse(rangeStart, formatter),
                    LocalDateTime.parse(rangeEnd, formatter),
                    PageRequest.of(from / size, size));
            ...
            eventsList = eventsRepository.getEventsWithUsersAndStatesAndCategories(users,
                    stateList,
                    categories,
                    LocalDateTime.parse(rangeStart, formatter),
                    LocalDateTime.parse(rangeEnd, formatter),
