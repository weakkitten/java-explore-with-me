...
public interface StatRepository extends JpaRepository<Hit, Integer> {
    ...
    "where (h.timestamp > ?1 and h.timestamp < ?2) + " +
            "and h.uri in ?3")
    public ViewStats getStats(LocalDateTime start, LocalDateTime end, List<String> uri);
    @Query("select h.app, h.uri, count(distinct h) from Hit h " +
            "where (h.timestamp > ?1 and h.timestamp < ?2) + " +
            "and h.uri in ?3")
    public ViewStats getStatsUnique(LocalDateTime start, LocalDateTime end, List<String> uri);
}
