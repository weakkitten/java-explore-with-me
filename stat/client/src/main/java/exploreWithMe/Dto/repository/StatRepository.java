...
public interface StatRepository extends JpaRepository<Hit, Integer> {
    ...
    @Query("select h.app, h.uri, count(h) from Hit h " +
            "where (h.timestamp > ?1 and h.timestamp < ?2) " +
            "and h.uri in ?3")
    public List<ViewStats> getStatsWithoutUris(LocalDateTime start, LocalDateTime end);
    @Query("select h.app, h.uri, count(distinct h) from Hit h " +
            "where (h.timestamp > ?1 and h.timestamp < ?2) " +
            "and h.uri in ?3")
    public List<ViewStats> getStatsUniqueWithoutUris(LocalDateTime start, LocalDateTime end);
}
